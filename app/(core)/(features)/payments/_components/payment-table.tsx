    ...
    <div className="w-full space-y-5">
        ...
        <div className="flex flex-col gap-3 w-full lg:flex-row lg:items-center lg:gap-3">
          ...
          <div className="flex items-center w-full lg:w-auto">
            <div className="relative w-full">
              ...
              <Input
                placeholder={`Pesquise por um pagamento específico...`}
                value={globalFilter}
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="received">Recebidos ({receivedCount})</TabsTrigger>
          <TabsTrigger value="pending">Pendentes ({pendingCount})</TabsTrigger>
          <TabsTrigger value="failed">Falhados({failedCount})</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <div className="flex flex-wrap items-center justify-between gap-5 py-4">
        <div className="flex-1 flex flex-row flex-wrap items-center gap-x-2 gap-y-1 text-sm text-muted-foreground whitespace-nowrap">
          <p className="text-foreground">
            A visualizar {table.getRowModel().rows.length} <span className="text-muted-foreground">de {table.getFilteredRowModel().rows.length} pagamento(s)</span>
          </p>
        </div>
